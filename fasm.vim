...
" Original Maintainer: Leonid V. Fedorenchik <leonid@fedorenchik.com>
" Last Authored: Cline + Gemini 2.5
" Last Change: 2025 May 10th
...
syn keyword fasmAddressSizes byte dqword dword fword large pword qword qqword dqqword zword
syn keyword fasmAddressSizes short tword tbyte word yword xword

syn keyword fasmDataDirectives db dd df dp dq dt du dw file rb rd rf rp rq rt rw emit dbx
syn keyword fasmDataDirectives rw rd rp rq rt rdq rqq rdqq
...
syn keyword fasmInstr endbr32 endbr64 clrssbsy rstorssp incsspd rdsspd incsspq rdsspq saveprevssp setssbsy wrssd wrussd wrssq wrussq
syn keyword fasmInstr getsec monitor mwait rdfsbase rdgsbase wrfsbase wrgsbase rdrand rdseed xacquire xrelease xtest xbegin xabort xend adcx adox movbe movdiri movdir64b

syn keyword fasmMacros         addr cinvoke endf endl endp ends feature frame invoke local locals proc stdcall struct uses macro endmacro struc endstruc calminstruction endcalminstruction
syn keyword fasmMacroDirective common forward local reverse
syn keyword fasmPreprocess     define equ fix forward include irp irps iterate enditerate rept repeat endrepeat while endwhile if elseif else endif
syn keyword fasmPreprocess     local macro match purge restore restruc struc reequ redefine postpone endpostpone virtual endvirtual namespace endnamespace
syn keyword fasmPreprocess     break indx eval esc mvmacro mvstruc outscope rawmatch rmatch
...
syn keyword fasmDirective      ms ms64 mz native near needed notpageable pe pe64 resource readable repeat section org
syn keyword fasmDirective      shareable static segment use16 use32 use64 virtual wdm writable writeable
syn keyword fasmDirective      bits16 bits32 bits64 immauto imm8 imm16 imm32 imm64 rmauto rm1 rmdst rm2 rmsrc
syn keyword fasmDirective      ripauto rip norip absolute use require load store restartout
syn keyword fasmDirective      retaincomments removecomments isolatelines combinelines

syn keyword fasmOperator       align as at defined eq eqtype from mod on ptr rva relativeto used
syn keyword fasmOperator       definite lengthof sizeof elementof scaleof metadataof elementsof float trunc string bappend
syn keyword fasmOperator       element scale metadata

syn keyword fasmCalmKeyword    assemble match jyes jno jump exit local arrange compute check publish transform stringify take call
syn keyword fasmCalmKeyword    display err emit load store " CALM versions

syn keyword fasmFeature        PCLMULQDQ MONITOR VMX SMX FMA CMPXCHG16B MOVBE POPCNT AESNI XSAVE F16C RDRAND FSGSBASE BMI1 HLE BMI2 RTM MPX
syn keyword fasmFeature        AVX512DQ RDSEED ADX AVX512_IFMA AVX512PF AVX512ER AVX512CD AVX512BW AVX512VL AVX512_VBMI AVX512_VBMI2 CET_SS
syn keyword fasmFeature        GFNI VAES VPCLMULQDQ AVX512_VNNI AVX512_BITALG AVX512_VPOPCNTDQ MOVDIRI MOVDIR64B AVX512_4VNNIW CET_IBT 3DNow
syn keyword fasmFeature        i8086 i186 i286 i386 i486 Pentium PentiumPro PentiumMMX Pentium2 Pentium3 Pentium4 AMD64 SSE4 AVX512 everything
syn keyword fasmFeature        x86 x87 MMX SSE AVX AVX_512 " From x86-2.inc defines

syn match   fasmSpecialLabel    "^\s*\(16\|32\|64\):"
...
hi def link fasmSpecialLabel    Label
hi def link fasmCalmKeyword     Keyword 
hi def link fasmFeature         Type
